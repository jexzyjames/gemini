...
const Sidebar = ({extended,setExtended}) => {
  ...
  let {prevPrompt,setPrevPrompt, newChat,onSent,setRecentPrompt} = useContext(Context);
  const {user} = useAuth()
  ...
  return (
    <div className='p-2 w-full fade h-screen hidden md:block m-0  bg-[#f0f4f9] '>
     <div className='flex   h-full flex-col justify-between items-center w-full '>
        ...
        <div>
        <img onClick={()=> setExtended((prev)=> !prev)} className=' cursor-pointer w-9' src={menuIcon} alt="" />
        {recent}
        </div>

        <div onClick={()=>newChat()}  className={`flex bg-slate-200 rounded-full items-center gap-2 px-2 justify-between ${
    extended ? 'w-full' : 'w-[50%]'
  }`}>
        ...
        {extended && <span className='chat inline-block font-bold w-full'>Recent</span>}
          ...
          <div className='flex overflow-y-scroll recent relative h-[300px] flex-col gap-2'>
            {prevPrompt?.map((query,id)=>{
              return(
               ...
               { extended && user && 
               <div onClick={()=> loadPrompt(query)} className='flex hover:bg-gray-300 px-1 w-full rounded-xl cursor-pointer gap-2 items-center'>
                  <div>
        ...
        <div className={` ${extended && '  justify-start items-center'} flex gap-2 justify-center  items-center`}>
        <img className=' cursor-pointer w-9' src={questionIcon} alt="" />
        {extended && <p>Question</p>}

        </div>

        <div className={` ${extended && '  justify-start flex-start items-center'} flex gap-2 justify-center  items-center`}>
        ...
        <img onClick={()=>{
          ...
          clearHistory()
        }} className=' cursor-pointer  w-9' src={historyIcon} alt="" />
